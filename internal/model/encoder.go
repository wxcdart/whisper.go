...
func NewEncoder(f gguf.FileLike) (*WhisperEncoder, error) {
	return NewEncoderWithBackend(f, nil)
}

// NewEncoderWithBackend loads an encoder from an open GGUF file using the provided compute backend.
// If backend is nil, the default ml backend is used.
func NewEncoderWithBackend(f gguf.FileLike, backend ComputeBackend) (*WhisperEncoder, error) {
...
func (e *WhisperEncoder) Encode(ctx context.Context, mel ml.Tensor) (ml.Tensor, error) {
	...
	x, err := backend.Conv1D(ctx, mel, e.conv1W, e.conv1B, 1)
	if err != nil {
		return ml.Tensor{}, fmt.Errorf("model: encoder: conv1: %w", err)
	}
	x = backend.GELU(x)

	x, err = backend.Conv1D(ctx, x, e.conv2W, e.conv2B, 2)
	if err != nil {
	...
	// Final layer norm.
	x = backend.LayerNorm(x, e.lnPostW, e.lnPostB, layerNormEps)
	return x, nil
}
...
func (e *WhisperEncoder) runSelfAttn(ctx context.Context, x ml.Tensor, a selfAttn, T, headDim int) (ml.Tensor, error) {
	...
	// Q = x @ qW^T + qB
	Q, err := matMulTransBMaybeQuant(ctx, backend, x, a.qW, a.qWQ)
	if err != nil {
		return ml.Tensor{}, fmt.Errorf("attn Q: %w", err)
	}
	Q = backend.Add(Q, a.qB)

	// K = x @ kW^T (no bias)
	K, err := matMulTransBMaybeQuant(ctx, backend, x, a.kW, a.kWQ)
	if err != nil {
		return ml.Tensor{}, fmt.Errorf("attn K: %w", err)
	}

	// V = x @ vW^T + vB
	V, err := matMulTransBMaybeQuant(ctx, backend, x, a.vW, a.vWQ)
	if err != nil {
		return ml.Tensor{}, fmt.Errorf("attn V: %w", err)
	}
	V = backend.Add(V, a.vB)

	// Reshape [T, D] → [T, nHead, headDim] → [nHead, T, headDim].
	Q = backend.Transpose(Q.Reshape(T, e.nHead, headDim), 1, 0, 2)
	K = backend.Transpose(K.Reshape(T, e.nHead, headDim), 1, 0, 2)
	V = backend.Transpose(V.Reshape(T, e.nHead, headDim), 1, 0, 2)

	// Scaled dot-product attention (no causal mask for encoder).
	attnOut, _, err := backend.ScaledDotProductAttention(ctx, Q, K, V, false, false)
	if err != nil {
		return ml.Tensor{}, fmt.Errorf("attn sdp: %w", err)
	}

	// [nHead, T, headDim] → [T, nHead, headDim] → [T, D].
	attnOut = backend.Transpose(attnOut, 1, 0, 2).Reshape(T, e.nAudioState)

	// Output projection: attnOut @ outW^T + outB.
	out, err := matMulTransBMaybeQuant(ctx, backend, attnOut, a.outW, a.outWQ)
	if err != nil {
		return ml.Tensor{}, fmt.Errorf("attn out: %w", err)
	}
	out = backend.Add(out, a.outB)
	return out, nil
}
...
func (e *WhisperEncoder) runMLP(ctx context.Context, x ml.Tensor, b *encoderBlock) (ml.Tensor, error) {
	...
	// Linear 1: [T, D] → [T, 4D]
	h, err := matMulTransBMaybeQuant(ctx, backend, x, b.mlp0W, b.mlp0WQ)
	if err != nil {
		return ml.Tensor{}, fmt.Errorf("mlp linear1: %w", err)
	}
	h = backend.Add(h, b.mlp0B)
	h = backend.GELU(h)

	// Linear 2: [T, 4D] → [T, D]
	out, err := matMulTransBMaybeQuant(ctx, backend, h, b.mlp2W, b.mlp2WQ)
	if err != nil {
		return ml.Tensor{}, fmt.Errorf("mlp linear2: %w", err)
	}
	out = backend.Add(out, b.mlp2B)
	return out, nil
}
