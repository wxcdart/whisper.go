...
func main(){
	...
	if idx < 0 { fmt.Println("not found"); return }
	...
	for i:=0;i<len(chunk);i+=16{
		line := chunk[i:]
		if len(line)>16{line=line[:16]}
		fmt.Printf("%08x  ", start+i)
		for j:=0;j<len(line);j++{fmt.Printf("%02x ", line[j])}
		for j:=len(line);j<16;j++{fmt.Print("   ")}
		fmt.Print(" ")
		for j:=0;j<len(line);j++{
			c := line[j]
			if c<32 || c>126 { fmt.Print('.') } else { fmt.Printf("%c", c) }
		}
		fmt.Println()
	}
}
